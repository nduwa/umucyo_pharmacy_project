  ...
  <div class="row" >
    <div class="col-sm-12 col-sm-offset-12">
      ...
      if(isset($_SESSION['message'])){
        ...
        echo '<script>
         setTimeout(function(){
            window.location.href = "view_basket";
         }, 2000);
      </script>';
      }
      ...
      ?>
      <form method="POST" action="save_basket">
        <div class="row">
          <?php  
      ...
      <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0" >
        ...
        <tbody>
          <?php
            //initialize total
          $sql_serve = "SELECT * FROM `total_sales` WHERE User_branch = '$_SESSION[branch]' AND institution_code = '$_SESSION[institution_code]' AND userID = '$_SESSION[user_id]' AND ser_status = '0' ORDER BY Ids DESC";
            $query = $conn->query($sql_serve);
            ...
            if($count > 0){
              ...
              while($row = $query->fetch_assoc()){
                ...
                $medecine_id  = $row['medicine_ID'];
                $batch_Number = $row['batch'];
                $ser_quantity = $row['quantity'];
                $sql_price = "SELECT * FROM `purchase_medical_product` INNER JOIN `medical_product` ON purchase_medical_product.product_name = medical_product.product_ID WHERE purchase_medical_product.product_name ='$medecine_id' AND purchase_medical_product.batch_Number = '$batch_Number' AND purchase_medical_product.purchase_status ='1'";
                $result_price = $conn->query($sql_price);
                $row_price = $result_price->fetch_assoc();

                $selling_price = $row['price'];

                $sub_total = $ser_quantity*$selling_price;

                $total =$total+$sub_total;

                // $_SESSION['medecine_id'][$index] = $row['medicine_ID'];
                // $_SESSION['batch_Number'][$index] = $row['batch'];
                ?>
                <tr>
                  <td>
                    <a href="delete_item.php?delete_item_id=<?php echo $item_id; ?>&&medecine_id=<?php echo $medecine_id; ?>&&batch=<?php echo $row['batch']; ?>" class="btn btn-danger btn-sm"><span class="fas fa-trash"></span></a>
                  </td>
                  <td><?php echo $row_price['product_Nmae']; ?></td>
                  <td>
                    <input type="hidden" name="indexes[]" value="<?php echo $index; ?>">
                    <input type="text" name="batch_Number" class="form-control" id="batch_Number" value="<?php echo $batch_Number; ?>" disabled="disabled" />
                  </td>
                  
                  <td >
                    <input type="text" class="form-control" id="selling_price" name="selling_price" value="<?php echo $selling_price; ?>" disabled="disabled" >         
                  </td>
     
                  <td>
                    <input type="text" class="form-control" id="ser_quantity" name="ser_quantity" value="<?php echo $ser_quantity; ?>" name="" min="0" value="0" step="any"required disabled="disabled"></td>

                  <td>
                    <input type="text" id="sub_total" name="sub_total" class="form-control" value="<?php echo $sub_total.' frw'; ?>" disabled="disabled" name="sub_total"> </td>
                </tr>
                <?php
                $total++;
                $index++;
          ...
          <tr>
            ...
            <td><b><?php $main_total = $total-$count;
             echo number_format($main_total, 2) . " <i>frw</i>"; ?></b></td>
          </tr>
        </tbody>
        <?php   
      ...
      <a href="clear_cart" class="btn btn-danger"><span class="fas fa-trash"></span> Clear Basket</a>
      <a href="checkout_basket.php?total=<?php echo $main_total; ?>&&trans=<?php echo $trans_id; ?>" class="btn btn-success <?php echo $_SESSION['checkout_btn']; ?>" ><span class="fas fa-check"></span>Checkout</a>
      </form>
    </div>
  </div>
</div>
